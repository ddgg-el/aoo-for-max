...
#define AOO_SAMPLE_SIZE 64

#include "ext.h"		// standard Max include, always required
#include "ext_obex.h"	// required for Max 4.5 and later style objects
...
// typedef not needed in C++
struct t_aoo_send {

	t_aoo_send(int argc, t_atom *argv);

	t_pxobject	x_obj; 		// audio object "base class"
	int32_t		x_nchannels = 1;
...
void aoo_send_list(t_aoo_send *x, t_symbol *s, int argc, t_atom *argv)
{
    ...
    argc -= 2; argv += 2;
    if (!argc) {
        object_error((t_object*)x, "%s: data must not be empty", classname(x));
        return;
    }
    ...
    if (auto err = x->x_source->addStreamMessage(msg); err != kAooOk) {
        object_error((t_object*)x, "%s: could not add stream message: %s",
                 classname(x), aoo_strerror(err));
    }
}
/**
 * @brief set float float handler
 * 
 * per quanto ne so fino ad adesso, questa funzione permette a aoo.send~ di rispondere
 *  al messaggio "set float float" e modificare in corsa la porta e l'id del nodo e l'id della source
 * 
 * 
 * @param x 
 * @param f1 
 * @param f2 
 */
void aoo_send_set(t_aoo_send *x, t_float f1, t_float f2)
{
    int port = f1;
    AooId id = f2;
    ...
    if (id < 0) {
        object_error((t_object*)x, "%s: bad id %d", classname(x), id);
        return;
    }

    if (port < 0) {
        // NB: 0 is allowed (= don't listen)!
        object_error((t_object*)x, "%s: bad port %d", classname(x), id);
        return;
    }
...
void aoo_send_id(t_aoo_send *x, t_float f)
{
    ...
    if (x->get_sink_arg(argc, argv, addr, id, false)){
        ...
        if (x->find_sink(addr, id)) {
            if (argv[1].a_type == A_SYM){
                ...
                object_error((t_object*)x, "%s: sink %s|%s %d already added!",
                         classname(x), group, user, id);
            } else {
                // host + port
                auto host = atom_getsym(argv)->s_name;
                object_error((t_object*)x, "%s: sink %s %d %d already added!",
        ...
        object_post((t_object*)x, "%s: added sink %s %d %d",
                classname(x), addr.name(), addr.port(), id);
    }
}
void aoo_send_remove(t_aoo_send *x, t_symbol *s, int argc, t_atom *argv)
{
    ...
    if (argc < 3){
        object_error((t_object*)x, "%s: too few arguments for 'remove' message", classname(x));
        return;
    }
    ...
    if (x->get_sink_arg(argc, argv, addr, id, true)) {
        ...
        object_post((t_object*)x, "%s: removed sink %s %d %d",
                classname(x), addr.name(), addr.port(), id);
    }
}
void aoo_send_active(t_aoo_send *x, t_symbol *s, int argc, t_atom *argv)
{
...
void aoo_send_start(t_aoo_send *x, t_symbol *s, int argc, t_atom *argv)
{
    ...
    if (argc > 0) {
        ...
        argc--; argv++;
        if (!argc) {
            object_error((t_object*)x, "%s: metadata must not be empty", classname(x));
            return;
        }
...
void aoo_send_stop(t_aoo_send *x)
{
    int32_t offset = clock_gettimesince(x->x_logicaltime) * 0.001 * x->x_samplerate;
...
void aoo_send_reset(t_aoo_send *x)
{
    x->x_source->reset();
}
void aoo_send_sink_list(t_aoo_send *x)
{
    ...
    for (auto& sink : x->x_sinks){
        ...
        if (x->x_node->serialize_endpoint(sink.s_address, sink.s_id, 3, msg)){
            ...
            error("t_node::serialize_endpoint");
        }
    }
}
void aoo_send_uninvite(t_aoo_send *x, t_symbol *s, int argc, t_atom *argv)
{
...
void aoo_send_format(t_aoo_send *x, t_symbol *s, int argc, t_atom *argv)
{
    AooFormatStorage f;
    if (format_parse((t_class *)x, f, argc, argv, x->x_nchannels)){
...
void aoo_send_sink_channel(t_aoo_send *x, t_symbol *s, int argc, t_atom *argv)
{
...
void aoo_send_packetsize(t_aoo_send *x, t_float f)
{
    x->x_source->setPacketSize(f);
}
void aoo_send_ping(t_aoo_send *x, t_float f)
{
    x->x_source->setPingInterval(f * 0.001);
}
void aoo_send_stream_time(t_aoo_send *x, t_float f)
{
    x->x_source->setStreamTimeSendInterval(f * 0.001);
}
void aoo_send_resend(t_aoo_send *x, t_float f)
{
    x->x_source->setResendBufferSize(f * 0.001);
}
void aoo_send_redundancy(t_aoo_send *x, t_float f)
{
    x->x_source->setRedundancy(f);
}
void aoo_send_resample_method(t_aoo_send *x, t_symbol *s)
{
...
void aoo_send_dynamic_resampling(t_aoo_send *x, t_float f)
{
    x->x_source->setDynamicResampling(f);
}
void aoo_send_dll_bandwidth(t_aoo_send *x, t_float f)
{
    x->x_source->setDllBandwidth(f);
}
void aoo_send_real_samplerate(t_aoo_send *x)
{
    AooSampleRate sr;
...
void aoo_send_binary(t_aoo_send *x, t_float f)
{
    x->x_source->setBinaryFormat(f);
}
void aoo_send_buffersize(t_aoo_send *x, t_float f)
{
    x->x_source->setBufferSize(f * 0.001);
}

// global class pointer variable
static t_class *s_aoo_send_class = NULL;
...
void t_aoo_send::add_sink(const aoo::ip_address& addr, AooId id)
{
    ...
    if (x_node->serialize_endpoint(addr, id, 3, msg)){
        ...
        error("t_aoo_send::add_sink: serialize_endpoint");
    }
}
    ...
    for (auto it = x_sinks.begin(); it != x_sinks.end(); ++it){
        if (it->s_address == addr && it->s_id == id){
            ...
            if (x_node->serialize_endpoint(addr, id, 3, msg)){
                ...
                error("aoo_send_doremovesink: serialize_endpoint");
            }
            return;
        }
    }
    error("t_aoo_send::remove_sink");
}
bool t_aoo_send::get_sink_arg(int argc, t_atom *argv,aoo::ip_address& addr, AooId& id, bool check) const
{
    if (!x_node) {
        object_error((t_object*)this, "%s: no socket!", classname((t_object*)this));
        return false;
    }
    if (argc < 3){
        object_error((t_object*)this, "%s: too few arguments for sink", classname((t_object*)this));
        return false;
    }
    ...
    if (argv[1].a_type == A_SYM) {
        ...
        object_error((t_object*)this, "%s: couldn't find sink %s|%s %d",
                 classname((t_object*)this), group->s_name, user->s_name, id);
        return false;
    } else {
        ...
        if (x_node->resolve(host, port, addr)) {
            if (check) {
                ...
                object_error((t_object*)this, "%s: couldn't find sink %s %d %d",
                         classname((t_object*)this), host->s_name, port, id);
                return false;
            } else {
                return true;
            }
        } else {
            object_error((t_object*)this, "%s: couldn't resolve sink hostname '%s'",
                     classname((t_object*)this), host->s_name);
            return false;
        }
    }
}
bool t_aoo_send::check(const char *name) const
{
    if (x_node){
        ...
        object_error((t_object*)this, "%s: '%s' failed: no socket!", classname((t_object*)this), name);
        return false;
    }
}
bool t_aoo_send::check(int argc, t_atom *argv, int minargs, const char *name) const
{
    ...
    if (argc < minargs){
        object_error((t_object*)this, "%s: too few arguments for '%s' message", classname((t_object*)this), name);
        return false;
    }
...
void t_aoo_send::remove_all()
{
    ...
    for (int i = 0; i < numsinks; ++i){
        ...
        if (x_node->serialize_endpoint(sinks[i].s_address, sinks[i].s_id, 3, msg)){
            ...
            error("aoo_send_doremoveall: serialize_endpoint");
        }
    }
}
/*
...
static bool get_opus_bitrate(t_aoo_send *x, t_atom *a) {
    ...
    if (err != kAooOk){
        object_error((t_object*)x, "%s: could not get bitrate: %s", classname(x), aoo_strerror(err));
        return false;
    }
...
static void aoo_send_handle_event(t_aoo_send *x, const AooEvent *event, int32_t)
{
    ...
    case kAooEventFrameResend:
    {
        ...
        if (!x->x_node->serialize_endpoint(addr, ep.id, 3, msg)) {
            error("aoo_send_handle_event: serialize_endpoint");
            return;
        }
        // event data
        switch (event->type){
        ...
        default:
            error("aoo_send_handle_event: bad case label!");
            break;
        }
...
static void aoo_send_port(t_aoo_send *x, t_float f)
{
    aoo_send_set(x, f, x->x_id);
}
static void aoo_send_auto_invite(t_aoo_send *x, t_float f) {
    x->x_auto_invite = f != 0;
}
...
t_aoo_send::t_aoo_send(int argc, t_atom *argv)
{
    ...
    //         //         x_multi = true;
    //         //     } else {
    //         //         error("%s: no multi-channel support, ignoring '-m' flag", classname(this));
    //         //     }
    //         // } else {
    //         //     error("%s: ignore unknown flag '%s",
    //         //              classname(this), flag);
    //         // }
    //         argc--; argv++;
    //     } else {
    ...
    if (x_multi) {
        ...
        if (ninlets < 0){
            ...
            error("%s: channel count (%d) out of range",
    ...
    if (id < 0) {
        error("%s: bad id % d, setting to 0", classname(this), id);
        id = 0;
    }
    x_id = id;
    ...
    // make event outlet
    x_msgout = outlet_new(&x_obj, NULL);
...
// aoo_send_tilde_setup in PD
// put it in a dedicated file to initialize all the externals at once
// it is actually important that all the plugins needs to share the same
// memory space
void ext_main(void *r)
{
	...
	aoo_send_class = class_new("aoo.send~", (method)aoo_send_new, (method)aoo_send_free, sizeof(t_aoo_send), NULL, A_GIMME, 0);

    class_addmethod(aoo_send_class, (method)aoo_send_list,"list", A_GIMME, 0);

	// Adds a method to a previously defined object class.
	class_addmethod(aoo_send_class, (method)aoo_send_dsp64,	"dsp64", 	A_CANT, 0);
	class_addmethod(aoo_send_class, (method)aoo_send_assist,	"assist",	A_CANT, 0);

    class_addmethod(aoo_send_class, (method)aoo_send_port,"port", A_FLOAT, 0);
    class_addmethod(aoo_send_class, (method)aoo_send_id,"id", A_FLOAT, 0);
    class_addmethod(aoo_send_class, (method)aoo_send_set, "set", A_DEFFLOAT, A_DEFFLOAT, 0);
    class_addmethod(aoo_send_class, (method)aoo_send_add,"add", A_GIMME, 0);
    class_addmethod(aoo_send_class, (method)aoo_send_remove,"remove", A_GIMME, 0);
    class_addmethod(aoo_send_class, (method)aoo_send_active,"active", A_GIMME, 0);
    class_addmethod(aoo_send_class, (method)aoo_send_start,"start", A_GIMME, 0);
    class_addmethod(aoo_send_class, (method)aoo_send_stop,"stop", 0);
    class_addmethod(aoo_send_class, (method)aoo_send_reset,"reset", 0);
    class_addmethod(aoo_send_class, (method)aoo_send_sink_list,"sink_list", 0);
    class_addmethod(aoo_send_class, (method)aoo_send_auto_invite,"auto_invite", A_FLOAT, 0);
    class_addmethod(aoo_send_class, (method)aoo_send_invite,"invite", A_GIMME, 0);
    class_addmethod(aoo_send_class, (method)aoo_send_uninvite,"uninvite", A_GIMME, 0);
    class_addmethod(aoo_send_class, (method)aoo_send_format,"format", A_GIMME, 0);
    class_addmethod(aoo_send_class, (method)aoo_send_codec_set,"codec_set", A_GIMME, 0);
    class_addmethod(aoo_send_class, (method)aoo_send_codec_get,"codec_get", A_SYM, 0);
    class_addmethod(aoo_send_class, (method)aoo_send_sink_channel,"sink_channel", A_GIMME, 0);
    class_addmethod(aoo_send_class, (method)aoo_send_packetsize,"packetsize", A_FLOAT, 0);
    class_addmethod(aoo_send_class, (method)aoo_send_ping,"ping", A_FLOAT, 0);
    class_addmethod(aoo_send_class, (method)aoo_send_stream_time,"stream_time", A_FLOAT, 0);
    class_addmethod(aoo_send_class, (method)aoo_send_resend,"resend", A_FLOAT, 0);
    class_addmethod(aoo_send_class, (method)aoo_send_redundancy,"redundancy", A_FLOAT, 0);
    class_addmethod(aoo_send_class, (method)aoo_send_resample_method,"resample_method", A_SYM, 0);
    class_addmethod(aoo_send_class, (method)aoo_send_dynamic_resampling,"dynamic_resampling", A_FLOAT, 0);
    class_addmethod(aoo_send_class, (method)aoo_send_dll_bandwidth,"dll_bandwidth", A_FLOAT, 0);
    class_addmethod(aoo_send_class, (method)aoo_send_real_samplerate,"real_samplerate", 0);
    class_addmethod(aoo_send_class, (method)aoo_send_binary,"binary", A_FLOAT, 0);
    class_addmethod(aoo_send_class, (method)aoo_send_buffersize,"buffersize", A_FLOAT, 0);
	
	class_dspinit(aoo_send_class);
	class_register(CLASS_BOX, aoo_send_class);
	s_aoo_send_class = aoo_send_class;
}

// aoo_send_new in PD
void *aoo_send_new(t_symbol *s, long argc, t_atom *argv)
{
	t_aoo_send *x = (t_aoo_send *)object_alloc(s_aoo_send_class);

	dsp_setup((t_pxobject *)x,x->x_nchannels);
	new (x) t_aoo_send(argc, argv);
	x->x_clock = clock_new(x,(method)aoo_send_tick);
	// x->p_value = x->p_max = 0;
...
// non in PD
void aoo_send_assist(t_aoo_send *x, void *b, long msg, long arg, char *s)
{
	if (msg == ASSIST_INLET)	//inlet
		sprintf(s, "(signal) Audio Input");
}

// aoo_send_perform in PD
void aoo_send_perform64(t_aoo_send *x, t_object *dsp64, double **ins, long numins, double **outs, long numouts, long sampleframes, long flags, void *userparam)
{
	t_double	*in = ins[0];
	t_double 	*out = outs[0];
	...
	/* IMPLEMENTAZIONE */
	// TODO: change AooSample to long
	static_assert(sizeof(long) == sizeof(AooSample),
					"AooSample size must match t_sample");
	
	if(x->x_node){
		// TODO: change AooSample to long
		auto err = x->x_source->process(x->x_vec.get(), xn, get_osctime());
		if (err == kAooErrorOverflow) {
            error("%s: send buffer overflow. Try to manually increase "
...
{
	/* IMPLEMENTAZIONE */
	// TODO: fix nchannel maybe in ext_main??
    int32_t nchannels;
    for (int i = 0; i < x->x_obj.z_in; i++)
    {
        if(count[i]!= 0){
            nchannels ++;
        }
	...
	if(maxvectorsize != x->x_blocksize || samplerate != x->x_samplerate || nchannels != x->x_nchannels) {
		x->x_source->setup(x->x_nchannels, samplerate, maxvectorsize, kAooFixedBlockSize);
	...
	/* FINE */
	// only put perform64 on dsp chain if sig is connected
