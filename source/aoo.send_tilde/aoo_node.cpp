...
{
    if ((t_class*)obj == aoo_client_class){
        // aoo_client
        if (!x_clientobj){
            x_clientobj = obj;
            // start thread lazily
    ...
    } else  if ((t_class*)obj == aoo_send_class){
    ...
    } else if ((t_class*)obj == aoo_receive_class){
        if (x_client->addSink((AooSink *)x) != kAooOk){
            object_error((t_object*)obj, "%s with ID %d on port %d already exists!",
                     classname(obj), id, port());
        }
    } else {
        error("t_node_imp: bad client");
        return false;
    }
    x_refcount++;
    return true;
...
void t_node_imp::send() {
    ...
    if (err != kAooOk) {
        ...
        sys_lock();
        object_error((t_object*)x_clientobj, "%s: UDP send error: %s",
                 (x_clientobj ? "aoo_client" : "aoo"), msg.c_str());
        // TODO: handle error
        sys_unlock();
    }
...
void t_node_imp::receive() {
    ...
    if (err != kAooOk) {
        ...
        sys_lock();
        object_error((t_object*)x_clientobj, "%s: UDP receive error: %s",
                 (x_clientobj ? "aoo_client" : "aoo"), msg.c_str());
        // TODO: handle error
        sys_unlock();
    }
...
t_node * t_node::get(t_class *obj, int port, void *x, AooId id)
{
    ...
    if (y) {
        ...
        try {
            ...
            object_error((object*)obj, "%s: %s", classname(obj), e.what());
            return nullptr;
        }
    }
    ...
    : x_proxy(this), x_bindsym(s), x_port(port)
{
    ...
    post(nullptr, PD_DEBUG, "aoo: new node on port %d", port);
}
...
void t_node_imp::release(t_class *obj, void *x)
{
    ...
    if (--x_refcount == 0){
        ...
        error("t_node_imp::release: negative refcount!");
    }
}
...
t_node_imp::~t_node_imp()
{
    ...
    post(nullptr, PD_DEBUG, "aoo: released node on port %d", x_port);
}

void aoo_node_setup(void)
{
    node_proxy_class = class_new("aoo node proxy", 0, 0,
                                 sizeof(t_node_proxy), CLASS_PD, 0);
}
