                               ...
                               const char *name, int32_t def)
{
    if (argc > which){
        ...
        if (s != gensym("_")){
            object_error((t_object*)x,"%s: bad %s argument (%s), using %d", classname(x), name, s->s_name, def);
        }
    #endif
    }
    return def;
...
bool format_parse(t_class *x, AooFormatStorage &f, int argc, t_atom *argv, int maxnumchannels)
{
    t_symbol *codec = atom_getsym(argv);
    ...
    if (codec == gensym(kAooCodecNull)){
        ...
        case 8:
            ...
            break;
        default:
            object_error((t_object*)x,"%s: bad bitdepth argument %d", classname(x), nbits);
            return false;
        }
        ...
        if (argc > 4){
            ...
            if (type == "_" || type == "audio") {
                ...
                object_error((t_object*)x,"%s: unsupported application type '%s'",
        ...
        object_error((t_object*)x,"%s: unknown codec '%s'", classname(x), codec->s_name);
        return false;
    }
    return true;
}
...
{
    if (argc < 4){
        error("format_to_atoms: too few atoms!");
        return 0;
    }
    ...
    if (codec == gensym(kAooCodecNull)){
        ...
        case kAooPcmFloat64:
            ...
            break;
        default:
            error(0, "format_to_atoms: bad bitdepth argument %d", fmt.bitDepth);
            return 0;
        }
...
bool atom_to_datatype(const t_atom &a, AooDataType& type, void *x) {
    if (a.a_type == A_SYM) {
        ...
        if (result != kAooDataUnspecified) {
            ...
            object_error((t_object*)x,"%s: unknown data type '%s'", classname(x), str);
        }
    } else {
        object_error((t_object*)x,"%s: bad metadata type", classname(x));
    }
    return false;
}
